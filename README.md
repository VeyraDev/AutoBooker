...
- 文件：本地持久化上传资料与生成图片；正式数据库目标仅支持 PostgreSQL + pgvector。
...
1. PostgreSQL 是业务状态的唯一事实来源。书稿、章节、文件生命周期、引用位置、优化修订和图片批次都持久化到数据库。
2. `Chapter.content` 是章节内容容器，保留纯文本并以 TipTap JSON 作为编辑器结构；引用、公式、表格和图片等富文本语义由节点表达。
3. 上传原文件和生成图片存放在文件系统，数据库只保存路径、解析状态和派生记录。多实例部署时必须提供共享卷，或将文件层替换为对象存储。
...
- 普通创作进入 `/app/books/:bookId`，由策划向导推进大纲和章节。
...
章节保存时，后端统一识别带文献 UUID 的内部标记、映射引用库并同步正文位置和上下文。图表同步从 Markdown 重建正文后会再次执行同一归一化过程，避免引用节点丢失。引用格式改变后，系统按正文首次出现顺序重新编号、以 `[1]` 等原子节点显示、刷新导出文字并重建书末参考文献；GB/T 7714 同一来源只列一次。无法匹配的标记进入“待补充来源”。

历史章节中的普通纯文本引用不会被自动改写；带 `[[CITE:...]]` 内部标记的章节会在读取章节或“本书引用”时自动转换。Markdown、DOCX 和 PDF 导出器都通过同一 TipTap 转换层识别结构化引用。
...
图片文件默认位于 `UPLOAD_DIR/figures`，由后端挂载为 `/static/figures`。
...
│  ├─ prompts/         # 写作、出版、记忆提取等提示约束
│  ├─ repositories/    # 数据访问辅助
│  ├─ routers/         # FastAPI 路由、鉴权和请求编排
│  ├─ schemas/         # Pydantic 请求与响应模型
│  ├─ services/        # 领域服务、导出、资料、引用、优化、图片与审校
│  └─ utils/           # JSON 修复、通用工具
...
| `/books/{id}/citations` | 引用库、节点、证据、正文位置和参考文献同步 |
| `/books/{id}/literature` | 查询优化、文献搜索、格式化与加入引用库 |
...
| 其他 | `GlobalLiterature`、`Notification`、`Feedback` |
...
| `UPLOAD_DIR` | 上传资料和默认图片根目录 |
| `FIGURES_DIR` | 可选的独立图片目录 |
...
- 生产环境应关闭 `--reload`，配置强 JWT 密钥、反向代理、HTTPS、共享文件存储和数据库备份。
- 当前后台任务依赖 API 进程。若需要多机调度、自动接管和独立扩缩容，应在后续版本接入外部任务队列。
