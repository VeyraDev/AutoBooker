...
export default function OutlineDrawer({ open, title = "大纲预览", onClose, children }: Props) {
  ...
  return createPortal(
    <div className="outline-drawer-overlay fixed inset-0 z-[300] flex h-dvh max-h-dvh min-h-0 flex-col bg-[color-mix(in_srgb,var(--bg-page)_92%,white)] backdrop-blur-lg">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-4 py-6 [-webkit-overflow-scrolling:touch]">
        {children}
      </div>
    </div>,
    document.body,
