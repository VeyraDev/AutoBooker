...
type Props = {
  ...
  externalSearch?: Record<string, unknown> | null;
};
...
export default function SourceLibraryPanel({
  ...
  const pickFromShelf = useCallback(
    async (item: LibraryShelfItem) => {
      let file: File;
      try {
        file = await fetchShelfItemAsFile(item);
      } catch (err) {
        toast.error(apiErrorMessage(err, "从书架下载失败"));
        throw err;
      }
      setShelfOpen(false);
  ...
  return (
    <div className="flex h-full flex-col border-l border-slate-200">
      <div className="border-b border-slate-200 px-3 py-2">
        ...
        >
          ...
          <div className="mt-3 flex flex-wrap justify-center gap-2">
            ...
            <button
              type="button"
              className="rounded border border-indigo-200 bg-indigo-50 px-2 py-1 text-xs text-indigo-700 disabled:opacity-50"
              disabled={busy}
      ...
      <div className="flex-1 overflow-y-auto p-3">
        {loading ? <div className="text-xs text-slate-500">加载资料…</div> : null}
        {error ? <div className="text-xs text-red-600">资料加载失败</div> : null}
        {!loading && !sources.length ? (
          <div className="text-xs text-slate-400">暂无资料，可上传、粘贴或从共享书架选择。</div>
        ) : null}
