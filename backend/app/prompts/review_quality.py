...
2. 事实、引用与参考文献：
   ...
   - 资料依据中标为 chapter_generation 的条目是本章生成时实际使用的资料；review_retrieval 是审校补充资料。判断“正文是否忠实使用资料”时优先检查前者。
   - 事实或引用问题命中资料依据时，basis_refs 必须写入对应“来源ID + 定位”；没有命中时明确标为 needs_verification，不得把补充检索结果伪装成正文原始依据。
3. 编校语言：
