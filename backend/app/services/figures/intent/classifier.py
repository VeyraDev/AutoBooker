...
from app.services.figures.catalog.type_catalog import CANONICAL_SUBTYPES, FIGURE_TYPE_CATALOG
...
- “甲→乙→丙→丁”的顺序链路是 workflow/process_flow，不是架构图。
- “甲、乙、丙模块之间的稳定关系”是 architecture/system_architecture。
- “变量、矩阵、权重、公式之间如何作用”是 knowledge/mechanism_diagram。
- “编码器、解码器或层级堆叠结构”是 knowledge/mechanism_diagram。
...
{{
  ...
  "diagram_subtype": "chart|timeline_roadmap|process_flow|system_architecture|mechanism_diagram|comparison_matrix|taxonomy_map|decision_tree|concept_diagram|infographic|scene_illustration|screenshot",
  "confidence": 0.0,
...
3. 系统模块、层级、服务关系 → system_architecture（旧别名均归此类，不要输出额外类型）。
4. 模型/算法/机制内部如何运作 → knowledge / mechanism_diagram（旧别名均归此类，不要输出额外类型）。
5. 两个或多个对象优劣/差异，含四象限 → matrix / comparison_matrix。
6. 分类体系、能力地图、思维导图、组织层级 → knowledge / taxonomy_map。
6b. 知识图谱、关系网络、概念语义关系 → knowledge / concept_diagram。
...
- "甲模块通过消息中介异步通知乙模块"是一条 relation，不影响 diagram_type 判断，不进 title。
...
def _sanitize_intent(data: dict[str, Any]) -> DiagramIntent | None:
    ...
    return DiagramIntent(
        FIGURE_TYPE_CATALOG.get(subtype).family if subtype in FIGURE_TYPE_CATALOG else family,
        subtype,
