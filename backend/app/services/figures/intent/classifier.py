...
- “RAG pipeline：提问→向量化→检索→生成”是 workflow/process_flow，不是架构图。
- “RAG 系统架构：用户、检索器、向量库、LLM 的模块关系”是 architecture/system_architecture。
- “Attention 权重矩阵 / Q-K scores / mask / softmax 可视化”是 matrix/attention_matrix。
- “Transformer 编码器-解码器堆叠结构”是 knowledge/mechanism_diagram。
...
{{
  ...
  "diagram_subtype": "chart|timeline_roadmap|process_flow|system_architecture|mechanism_diagram|comparison_matrix|swot|attention_matrix|taxonomy_map|decision_tree|concept_diagram|infographic|scene_illustration",
  "confidence": 0.0,
...
3. 系统模块、层级、服务关系 → system_architecture（RAG/Agent/微服务均归此类，不要单独输出 rag/agent）。
4. 模型/算法/机制内部如何运作 → knowledge / mechanism_diagram（Transformer 归此类，不要单独输出 transformer）。
