...
def _create_wanx_task(client: httpx.Client, prompt: str, model: str) -> str:
...
def generate_figure_image_wanx(
    ...
    *,
    ...
    with httpx.Client(timeout=60.0) as client:
        task_id = _create_wanx_task(client, prompt, model)
        logger.info("wanx task created model=%s task_id=%s", model, task_id)
