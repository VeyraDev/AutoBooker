...
def default_ai_model() -> str:
    """返回首个已配置服务商的默认模型（优先 DeepSeek，其次千问）。"""
    for preferred in ("deepseek", "qwen"):
        spec = get_provider_spec(preferred)
        if spec and is_provider_configured(preferred):
            default_model = (
                settings.DEEPSEEK_CHAT_MODEL if preferred == "deepseek" else settings.CHAT_MODEL
            )
        ...
        providers_out.append(
            {
                ...
                "label": spec.label,
                "region": spec.region,
                "models": [{"id": m, "label": m} for m in spec.models],
            }
        )
    return {
