"""Repair writing_bases schema if t8 migration was stamped but not applied."""
...
def upgrade() -> None:
    if not _enum_exists("writing_basis_status"):
        op.execute(
            "CREATE TYPE writing_basis_status AS ENUM ('draft', 'confirmed', 'superseded')"
        )

    if not _table_exists("writing_bases"):
        op.create_table(
            "writing_bases",
    ...
    if not _column_exists("project_intakes", "confirmed_writing_basis_id"):
        op.add_column(
            "project_intakes",
            sa.Column("confirmed_writing_basis_id", postgresql.UUID(as_uuid=True), nullable=True),
        )

    if _table_exists("generation_context_snapshots") and not _column_exists(
        "generation_context_snapshots", "writing_basis_id"
    ):
        op.add_column(
...
def downgrade() -> None:
    if _column_exists("generation_context_snapshots", "writing_basis_id"):
        op.drop_column("generation_context_snapshots", "writing_basis_id")
    if _column_exists("project_intakes", "confirmed_writing_basis_id"):
        op.drop_column("project_intakes", "confirmed_writing_basis_id")
    if _table_exists("writing_bases"):
    ...
    if _enum_exists("writing_basis_status"):
